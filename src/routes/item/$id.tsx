          ...
          <div className="rounded-xl border border-slate-700/80 bg-slate-900/50 p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400">
                Price by float bucket
              </h2>
              <span className="text-[10px] text-slate-500">
                {bucketsWithData.length} / {buckets.length} buckets
              </span>
            </div>
                  ...
                  onClick={(d) => {
                    if (d?.activePayload?.[0]) {
                      ...
                      setSelected({
                        ...
                        price: pt.price != null ? pt.price * 100 : null,
                      });
                    }
                  }}
                >
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-[#0d1219] border border-slate-700/80 rounded-lg px-3 py-2 text-xs shadow-xl">
                          ...
                          <p className="mono font-bold text-emerald-300 text-sm">
                            ${d.price?.toFixed(2)}
                          </p>
                        </div>
                      );
                        ...
                        fill={
                          entry.isEmpty
                            ...
                            : selected?.floatMin === entry.floatMin
                              ...
                              : entry.price != null &&
                                  maxPrice != null &&
                                  entry.price / 100 >= (maxPrice / 100) * 0.95
                                ? "#34d399"
                                : "#1e40af"
                        }
          ...
          {minPrice != null && (
            <div className="rounded-xl border border-slate-700/80 bg-slate-900/50 p-4 space-y-3">
              {[
                ...
                {
                  label: "premium",
                  value: premium ? `+${premium.toFixed(1)}%` : "—",
                  color:
                    premium && premium >= 20
                      ? "text-cyan-300"
                      : "text-slate-400",
                ...
                >
                  ...
                  <span className={`mono text-sm font-semibold ${s.color}`}>
                    {s.value}
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-0.5">
              {buckets.map((b) => (
                ...
                >
                  ...
                  <span
                    className={`mono text-[11px] font-medium ${b.price != null ? "text-emerald-400" : "text-slate-700"}`}
                  >
